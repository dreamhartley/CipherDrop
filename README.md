...
- **智能加密策略**：小于100MB文件端到端加密，大文件服务端自动销毁
- **会话隔离**：每个传输会话独立，互不干扰
...
- **无文件类型限制**：支持所有文件格式
- **大文件支持**：自动分块传输，支持超大文件
- **智能处理**：超过100MB文件自动跳过端到端加密，提升传输效率
- **实时进度**：上传下载进度实时显示
- **断点续传**：分块上传失败自动重试
...
1. 拖拽文件到传输区域，或点击上传按钮
2. 文件自动处理并上传：
   - **小文件（≤100MB）**：端到端加密传输，最高安全性
   - **大文件（>100MB）**：直接传输，会话结束后自动销毁
3. 对方可实时看到文件并下载
4. 支持图片、视频、音频预览
...
### 安全特性
- **AES-GCM加密**：256位密钥长度（小于100MB文件）
- **随机密钥生成**：每个文件独立密钥
- **智能安全策略**：根据文件大小自动选择最佳安全方案
...
### 上传配置
- **单文件大小限制**：无限制（受存储空间限制）
- **端到端加密阈值**：100MB（超过此大小跳过加密）
- **分块上传阈值**：80MB（超过此大小使用分块传输）
- **分块大小**：50MB
- **并发上传数**：3个分块
- **会话存储限制**：1GB（可配置为无限制）
